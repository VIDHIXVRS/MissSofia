...
import Sofia.modules.sql.blacklist_sql as blacklistsql
import Sofia.modules.sql.locks_sql as locksql
import Sofia.modules.sql.notes_sql as sql
import Sofia.modules.sql.rules_sql as rulessql
from Sofia import JOIN_LOGGER, LOGGER, OWNER_ID, SUPPORT_CHAT, dispatcher
from Sofia.__main__ import DATA_IMPORT
from Sofia.modules.connection import connected
from Sofia.modules.helper_funcs.alternate import typing_action
from Sofia.modules.helper_funcs.chat_status import user_admin
from Sofia.modules.sql import disable_sql as disabledsql
...
def export_data(update, context):
    ...
    with open("SofiaRobot{}Backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("SofiaRobot{}Backup".format(chat_id), "rb"),
        caption="*sᴜᴄᴄᴇssғᴜʟʟʏ ᴇxᴘᴏʀᴛᴇᴅ ʙᴀᴄᴋᴜᴘ:*\nᴄʜᴀᴛ: `{}`\nᴄʜᴀᴛ ɪᴅ: `{}`\nᴏɴ: `{}`\n\nɴᴏᴛᴇ: ᴛʜɪs `ꜱᴏꜰɪᴀʀᴏʙᴏᴛ-ʙᴀᴄᴋᴜᴘ` ᴡᴀs sᴘᴇᴄɪᴀʟʟʏ ᴍᴀᴅᴇ ғᴏʀ ɴᴏᴛᴇs.".format(
            chat.title,
            chat_id,
            tgl,
        ),
    ...
    os.remove("Sofia{}Backup".format(chat_id))  # Cleaning file
