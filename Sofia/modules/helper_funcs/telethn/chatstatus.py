...
from Sofia import DRAGONS
from Sofia.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def Sofia_is_admin(chat_id: int):
    status = False
    Sofia = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id,
        filter=ChannelParticipantsAdmins,
    ):
