...
from Sofia import DEV_USERS, OWNER_ID, dispatcher, telethn
from Sofia.modules.helper_funcs.chat_status import dev_plus
...
def allow_groups(update: Update, context: CallbackContext):
    args = context.args
    if not args:
        state = "off" if Sofia.ALLOW_CHATS else "ʟᴏᴄᴋᴅᴏᴡɴ ɪs " + "on"
        update.effective_message.reply_text(f"ᴄᴜʀʀᴇɴᴛ sᴛᴀᴛᴇ: {state}")
        ʀᴇᴛᴜʀɴ
    if args[0].lower() in ["off", "no"]:
        Sofia.ALLOW_CHATS = True
    elif args[0].lower() in ["yes", "on"]:
        Sofia.ALLOW_CHATS = False
    else:
...
def leave(update: Update, context: CallbackContext):
    ...
    if args := context.args:
        ...
        update.effective_message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup(Sofia_leave_bt),
        )
...
def restart(update: Update, context: CallbackContext):
    ...
    process = subprocess.run("pkill python3 && python3 -m Sofia", shell=True, check=True)
    process.communicate()
