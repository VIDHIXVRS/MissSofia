...
import Sofia.modules.sql.feds_sql as sql
from Sofia import DRAGONS, EVENT_LOGS, LOGGER, OWNER_ID, TIGERS, WOLVES, dispatcher
from Sofia.modules.disable import DisableAbleCommandHandler
from Sofia.modules.helper_funcs.alternate import send_message
...
from Sofia.modules.helper_funcs.string_handling import markdown_parser
...
def new_fed(update: Update, context: CallbackContext):
    ...
    if fednam != "":
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't ғᴇᴅᴇʀᴀᴛᴇ! ᴘʟᴇᴀsᴇ ᴄᴏɴᴛᴀᴄᴛ @SOFIASUPPORT if the problem persist."
            )
            return
...
def join_fed(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please contact @SOFIASUPPORT should this problem persist!"
            )
            return
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "Sofia_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="Sofia_fbanned_users.json",
